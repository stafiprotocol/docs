---
title: StaFi Protocol Product
---

<Callout type="info">
StaFi App has been upgraded to the latest version. To learn more about the updates, please visit https://app.stafi.io/.
</Callout>
...
[rToken App](https://app.stafi.io/) supports the staking features for following tokens: ETH(Ethereum), FIS(StaFi), BNB(BSC), MATIC(Polygon), ATOM(Cosmos), SOL(Solana), DOT(Polkadot), KSM(Kusama) and so on. 

Users can easily deposit their tokens to a stake pool in the rToken App, just like swapping, and receive rTokens in return.

## Pool

[rPool](https://app.stafi.io/pool/evm?net=ERC20)  is a staking incentive program initiated by StaFi. Users who stake their tokens through the StaFi rToken App can participate in rPool and earn **FIS tokens** in proportion to their stake.
...
[rBridge](https://app.stafi.io/gallery/evm/ETH?net=ERC20&tab=bridge&srcTokenStandard=ERC20&dstTokenStandard=Polygon&tokenName=rETH) is a cross-chain bridge that allows users to transfer rTokens between the StaFi Chain, StaFi Hub and the original chain. This makes it possible for users to use their rTokens in DeFi applications that are built on the original chain.

![StaFi rPool](/image/stakingoverview_1.png)

## Asset

[rAsset](https://app.stafi.io/profile/evm?net=ERC20) is a dashboard product for rToken holders. It allows rToken holders to manage their rToken assets, including the amount of rTokens in their wallet, the total value of their held rTokens, and the number of redeemable native tokens.